...
export async function getUpcomingEvents({ teamId, fetchImpl = globalThis.fetch } = {}) {
  ...
  const upcomingEvents = uniqueEvents
    ...
    .sort((a, b) => {
      const dateA = a.timestamp ? new Date(a.timestamp + (a.timestamp.includes('Z') || /[-+]\d{2}:?\d{2}$/.test(a.timestamp) ? '' : 'Z')) : new Date(`${a.date}T${a.time || '00:00:00'}Z`);
      const dateB = b.timestamp ? new Date(b.timestamp + (b.timestamp.includes('Z') || /[-+]\d{2}:?\d{2}$/.test(b.timestamp) ? '' : 'Z')) : new Date(`${b.date}T${b.time || '00:00:00'}Z`);
      return dateA - dateB;
    });
