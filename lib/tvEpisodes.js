...
import { escapeIcsText, foldIcsLine, formatIcsUtc, normalizeLastModified, sequenceFromLastModified } from './utils/ics.js';
...
function normalizeEpisode(episode, show) {
  return {
    ...
    network: show.network?.name || show.webChannel?.name || null,
    country: show.network?.country?.name || show.webChannel?.country?.name || null,
    updatedAt: episode.updatedAt || episode.updated || episode.lastModified || episode.modified || null
  };
}
    ...
    for (let i = 0; i < allEpisodes.length; i++) {
      ...
      if (currentEligible) {
        ...
        while (j < allEpisodes.length) {
          ...
          if (nextEligible && next.airdate === current.airdate && next.season === current.season) {
...
export function toIcs(result, { timezone = 'UTC' } = {}) {
  ...
  for (const episode of result.episodes) {
    ...
    const lastModified = normalizeLastModified(episode.updatedAt);
    lines.push(
      'BEGIN:VEVENT',
      ...
      `SEQUENCE:${sequenceFromLastModified(lastModified)}`,
      'STATUS:CONFIRMED',
      `DTSTART:${start}`,
      `DTEND:${end}`,
      `SUMMARY:${escapeIcsText(`${episode.showName} ${episodeCode} ${episode.name}`.replace(/\s+/g, ' ').trim())}`,
      `DESCRIPTION:${escapeIcsText(formatDescription(episode, timesString))}`,
  ...
  return `${lines.map((line) => {
    const imdbDetail = line.match(/IMDb Episode: https?:\/\/\S+/)?.[0];
    return foldIcsLine(line, { keepTogether: imdbDetail ? [imdbDetail] : [] });
  }).join('\r\n')}\r\n`;
}
