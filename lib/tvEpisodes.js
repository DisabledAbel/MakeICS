...
async function fetchJson(url, fetchImpl) {
  ...
  if (process.env.IMDB_API_URL) {
    try { trustedHosts.push(new URL(process.env.IMDB_API_URL).hostname); } catch (e) {}
  }
  if (process.env.FIRECRAWL_API_URL) {
    try { trustedHosts.push(new URL(process.env.FIRECRAWL_API_URL).hostname); } catch (e) {}
  }
...
async function fetchPublicImdbDetails(imdbId, env, fetchImpl, fallbackWarning = null) {
  ...
  try {
    ...
    const trustedHosts = ['imdb.iamidiotareyoutoo.com', 'api.firecrawl.dev'];
    if (env && env.IMDB_API_URL) {
      try { trustedHosts.push(new URL(env.IMDB_API_URL).hostname); } catch (e) {}
    }
    const host = parsedUrl.hostname.toLowerCase();
    const isAllowed = trustedHosts.includes(host) ||
                      host.endsWith('.imdb.com') ||
...
async function fetchImdbEpisodesForSeasons(imdbId, allEpisodes, fetchImpl, env) {
  ...
  const fetchPromises = sortedSeasons.map(async (sNum) => {
    try {
      const eps = await fetchImdbEpisodesFn(imdbId, sNum, fetchImpl, env);
