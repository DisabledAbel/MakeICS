...
async function getShowById(showId, fetchImpl) {
  const url = `${TVMAZE_BASE_URL}/shows/${encodeURIComponent(showId)}?embed=nextepisode`;
  return fetchJson(url, fetchImpl);
}
...
async function fetchPublicImdbDetails(imdbId, env, fetchImpl, fallbackWarning = null) {
  ...
  if (!response.ok) {
    return {
      ...
      source: 'public-imdb',
      sourceConfigured: true,
      error: `IMDb endpoint returned HTTP ${response.status}`,
      warning: fallbackWarning
    };
  }

  const payload = await response.json();
  return normalizeImdbApiPayload(payload, imdbId, env.IMDB_API_URL ? 'custom-imdb-endpoint' : 'public-imdb', fallbackWarning);
}
...
export async function getUpcomingEpisodes({ query, showId, now = new Date(), fetchImpl = globalThis.fetch, env = process.env } = {}) {
  ...
  // Prefer showId for stable, deterministic lookups
  if (showId) {
    const trimmedId = String(showId).trim();
    if (!trimmedId) {
      const error = new Error('A show ID is required.');
      error.statusCode = 400;
      throw error;
    }
    show = await getShowById(trimmedId, fetchImpl);
    lookupValue = trimmedId;
  } else {
    // Fall back to fuzzy query-based search
