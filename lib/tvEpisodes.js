...
export async function getEpisodes({ query, now = new Date(), since = null, fetchImpl = globalThis.fetch, env = process.env } = {}) {
  ...
  const imdb = await fetchImdbDetails(imdbId, env, fetchImpl);
    ...
    try {
      const verifiedPath = path.join(__dirname, 'data/tv/google-verified.json');
      const verifiedContent = await fs.readFile(verifiedPath, 'utf8');
  ...
  try {
    ...
    if (cachedRtData && cachedRtData.shows) {
      ...
      cachedShow = Object.values(cachedRtData.shows).find(s => {
    ...
    if (cachedShow) {
      console.log(`Using pre-cached Rotten Tomatoes data for "${trimmedQuery}"`);
      rtShow = cachedShow;
      ...
      if (rtShow) {
        ...
        const fetchPromises = sortedSeasons.map(async (sNum) => {
          try {
            const eps = await fetchRtEpisodes(rtShow.slug, sNum, fetchImpl, env);
    ...
    if (rtShow) {
      ...
      allEpisodes.forEach(ep => {
        if (ep.season && ep.number) {
          tvmazeEpMap.set(`${ep.season}-${ep.number}`, ep);
        }
      });
  ...
  if (googleVerifiedData) {
    allEpisodes.forEach(ep => {
      const overrideKey = `${show.name}-${ep.season}-${ep.number}`;
      const override = googleVerifiedData[overrideKey];
      if (override) {
        if (override.airdate) {
          ep.airdate = override.airdate;
          if (ep.airstamp && typeof ep.airstamp === 'string' && /^\d{4}-\d{2}-\d{2}T/.test(ep.airstamp)) {
            ep.airstamp = override.airdate + ep.airstamp.substring(10);
          } else {
            const airtimeStr = ep.airtime || '00:00';
            const formattedTime = airtimeStr.split(':').length === 2 ? `${airtimeStr}:00` : airtimeStr;
            ep.airstamp = `${override.airdate}T${formattedTime}Z`;
          }
        }
        if (override.name) {
          ep.name = override.name;
        }
      }
    });
