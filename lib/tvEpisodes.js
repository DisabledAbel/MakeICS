...
export async function getEpisodes({ query, now = new Date(), since = null, fetchImpl = globalThis.fetch, env = process.env } = {}) {
  ...
  if (isChildrenShow(show)) {
    ...
    for (let i = 0; i < allEpisodes.length; i++) {
      ...
      if (next &&
          current.airdate === next.airdate &&
          current.season === next.season &&
          (current.runtime === null || current.runtime < 20) &&
          (next.runtime === null || next.runtime < 20)) {

        // Merge them!
        const mergedEp = {
          ...current,
          name: `${current.name}/${next.name}`,
          runtime: (current.runtime || 0) + (next.runtime || 0),
          summary: [current.summary, next.summary].filter(Boolean).join(' '),
          url: current.url
        };
        merged.push(mergedEp);
        i++; // skip next
      } else {
        merged.push(current);
      }
