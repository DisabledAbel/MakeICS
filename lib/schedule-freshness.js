...
export function evaluateSource(source, { now, lastSuccess, signal }) {
  ...
  if (fetchAgeDays > source.fetchGraceDays) {
    reasonCategories.push('fetch-age');
    reasons.push(lastSuccess ? `last successful workflow run was ${Math.floor(fetchAgeDays)} days ago` : 'no successful workflow run was found');
  }
  if (active && horizonDays < source.horizonDays) {
    reasonCategories.push('data-horizon');
    reasons.push(signal.newest ? `newest scheduled item is only ${Math.floor(horizonDays)} days ahead` : 'no dated schedule items were found');
  }
  return {
