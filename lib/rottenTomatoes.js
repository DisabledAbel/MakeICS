  ...
  for (const ep of episodes) {
    ...
    if (!map.has(key)) {
      ...
      map.set(key, {
        season: ep.season,
        number: ep.number,
        name: existing.name || ep.name,
        airdate: existing.airdate || ep.airdate,
        summary: existing.summary || ep.summary,
        url: existing.url || ep.url
      });
    }
  }
...
export async function searchRtShow(query, fetchImpl = globalThis.fetch) {
  ...
  try {
    ...
    let bestMatch = shows.find(s => canonicalize(s.title) === canonicalQuery);
    ...
    if (!bestMatch) {
      ...
      const substringMatches = shows.filter(s => {
        const canonicalShowTitle = canonicalize(s.title);
      ...
      if (substringMatches.length > 0) {
        ...
        if (queryYear) {
          const matchingYearShow = substringMatches.find(s => s.startYear === queryYear);
          if (matchingYearShow) {
