                 ...
                 groups.forEach(groupObj => {
                  (groupObj.group || []).forEach(event => {
                    results.push({
                      ...
                      homeTeam: event.opponent?.homeAwaySymbol === '@' ? event.opponent?.displayName : teamDisplayName,
                      awayTeam: event.opponent?.homeAwaySymbol === '@' ? teamDisplayName : event.opponent?.displayName,
    ...
    const domResults = await page.evaluate((teamSlug) => {
      ...
      return rows.map(row => {
        ...
        return {
          ...
          // Fix: Use display names
          homeTeam: isAway ? opponentName : teamDisplayName,
          awayTeam: isAway ? teamDisplayName : opponentName,
          venue: null,
          league: null
        };
      }).filter(Boolean);
        ...
        if (text.includes('schedule') || text.includes('opponent') || text.includes('game')) {
          ...
          if (data.length > 5) {
             ...
             return data.map(row => ({
               date: row[0],
               name: row[1],
               // Fix: Specific time detection regex
               time: row.find(c => c && timeRegex.test(c)),
