  ...
  if (process.env.NODE_ENV === 'test') {
    return {
      newPage: async () => ({
        ...
        content: async () => '<html><script type="application/ld+json">{"@type":"TVEpisode","episodeNumber":3,"name":"Future episode.","datePublished":"2026-06-11","description":"Future episode."}</script></html>',
        evaluate: async () => [
          { season: 2, number: 3, name: "Future episode.", airdateStr: "2026-06-11", summary: "Future episode.", url: "https://www.rottentomatoes.com/tv/example_show/s02/e03" }
        ],
        close: async () => {}
      }),
      close: async () => {}
        ...
        if (text.includes('schedule') || text.includes('opponent') || text.includes('game')) {
          ...
          if (rows.length > 5) {
             ...
             for (const row of rows) {
               ...
               if (date.toLowerCase() === 'date' || name.toLowerCase() === 'opponent' || name.toLowerCase() === 'matchup') {
                 continue;
               }
