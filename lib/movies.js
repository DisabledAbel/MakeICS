...
export async function searchMovieSuggestions({ query, type = 'all', fsImpl = fs } = {}) {
  ...
  for (const movie of movies) {
    ...
    if (type === 'all' || type === 'movie' || type === 'studio') {
      if (movie.title.toLowerCase().includes(trimmedQuery)) {
        match = true;
      }
    }

    if (!match && (type === 'all' || type === 'genre' || type === 'studio')) {
    ...
    if (!match && (type === 'all' || type === 'character' || type === 'people' || type === 'studio')) {
...
export async function getMovies({ type = 'all', query = '', since = null, now = new Date(), fsImpl = fs } = {}) {
  ...
  if (trimmedQuery) {
    filtered = movies.filter(movie => {
      ...
      // because the source data does not have a dedicated production company field.
