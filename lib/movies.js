...
export function toIcs(result) {
  ...
  for (const movie of result.movies) {
    ...
    const lastModified = normalizeLastModified(movie.updatedAt || movie.updated || movie.lastModified || result.generatedAt);
    lines.push(
      'BEGIN:VEVENT',
      ...
      `STATUS:${normalizeEventStatus(movie.status)}`,
      `DTSTART;VALUE=DATE:${start}`,
      `SUMMARY:${escapeIcsText(movie.title)} (Movie Release)`,
