...
export function parseImdbDate(dateStr) {
  ...
  // Reject year-only or month-year placeholders (e.g. "2026" or "August 2026" or "Aug, 2026")
  ...
  if (isoMatch) {
    ...
    return isValidImdbAirdate(date) ? date : null;
  }
  ...
  // Date.parse normalizes some impossible numeric dates (for example,
  // 02/30/2014 becomes March 2), so validate their source components first.
  const numericMdyMatch = trimmed.match(/^(\d{1,2})[/-](\d{1,2})[/-](\d{4})(?:\D|$)/);
  const numericYmdMatch = trimmed.match(/^(\d{4})[/-](\d{1,2})[/-](\d{1,2})(?:\D|$)/);
  if (numericMdyMatch || numericYmdMatch) {
    const year = numericYmdMatch?.[1] || numericMdyMatch[3];
    const month = numericYmdMatch?.[2] || numericMdyMatch[1];
    const day = numericYmdMatch?.[3] || numericMdyMatch[2];
    const sourceDate = `${year}-${month.padStart(2, '0')}-${day.padStart(2, '0')}`;
    if (!isValidImdbAirdate(sourceDate)) return null;
  }

  // Append ' UTC' to interpret inputs as UTC timezone before extracting UTC date fields.
  let parsed = Date.parse(trimmed + ' UTC');
...
/** IMDb cache records must have a real, complete calendar date. */
export function isValidImdbAirdate(value) {
  if (typeof value !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(value)) return false;
  const date = new Date(`${value}T00:00:00Z`);
  return Number.isFinite(date.getTime()) && date.toISOString().slice(0, 10) === value;
}

/** Remove unschedulable entries without changing valid episode objects or URLs. */
export function sanitizeImdbEpisodes(episodes) {
  if (!Array.isArray(episodes)) return [];
  return episodes.filter((episode) =>
    episode && isValidImdbAirdate(episode.airdate)
  );
}
...
export async function fetchImdbEpisodes(imdbId, seasonNumber) {
  ...
  try {
    ...
    try {
      await page.waitForSelector('[data-testid="episode-item-wrapper"], .episode-item-wrapper, .list_item, .info, script[type="application/ld+json"]', { timeout: 15000 });
    } catch (waitError) {
      try {
    ...
    try {
      const domEpisodes = await page.evaluate((sNum) => {
        ...
        items.forEach(item => {
          let name = null;
          let titleEl = null;
          const titleSelectors = ['.ipc-title__text', '[data-testid="slate-list-card-title"]', '[data-testid="episode-item-title"]', 'h4', 'h3', 'strong a', 'a[href*="/title/tt"]'];
          ...
          if (numEl) {
            ...
            const numMatch = text.match(/Ep\s*(\d+)/i) || text.match(/Episode\s*(\d+)/i) || text.match(/S\d+,\s*Ep\s*(\d+)/i) || text.match(/\.E\s*(\d+)/i) || text.match(/E\s*(\d+)/i);
            if (numMatch) number = Number(numMatch[1]);
          }

          if (number === null && name) {
            const epMatch = name.match(/(?:Episode|Ep\.|Ep)\s*(\d+)/i) || name.match(/\.E\s*(\d+)/i) || name.match(/E\s*(\d+)/i);
            if (epMatch) number = Number(epMatch[1]);
          }
          if (number === null) {
            const text = item.innerText || '';
            const epMatch = text.match(/(?:Episode|Ep\.|Ep)\s*(\d+)/i) || text.match(/\.E\s*(\d+)/i) || text.match(/E\s*(\d+)/i);
            if (epMatch) number = Number(epMatch[1]);
          }

          if (name) {
            name = name.replace(/^S\d+\.E\s*\d+\s*(?:∙|:|-)?\s*/i, '').trim();
            name = name.replace(/^(?:Episode|Ep\.)\s*\d+\s*(?:∙|:|-)?\s*/i, '').trim();
          }

          const airEl = item.querySelector('[data-testid="episode-item-air-date"], .airdate, [class*="air-date"]') || item.querySelector('h4 + span');
          const airdateStr = airEl ? airEl.innerText.trim() : null;

          const synopsisEl = item.querySelector('[data-testid="episode-item-description"], .item_description, p, .ipc-html-content-inner-div');
          ...
          if (titleEl) {
            ...
            if (!href) {
              const anchor = titleEl.closest('a') || titleEl.querySelector('a');
              if (anchor) href = anchor.getAttribute('href');
            }
          }
          if (!href) {
            const links = Array.from(item.querySelectorAll('a[href*="/title/tt"]'));
            const linkEl = links.find(l => l.getAttribute('href'));
    ...
    return sanitizeImdbEpisodes(episodes);
  } catch (error) {
