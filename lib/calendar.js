...
import { createEventFingerprint, escapeIcsText, foldIcsLine, formatIcsUtc, normalizeEventStatus, normalizeLastModified, sequenceFromLastModified } from './utils/ics.js';
...
function revisionFields(item, defaultStatus = 'CONFIRMED', fallbackLastModified = null) {
  const sourceStatus = item.status == null ? null : String(item.status);
  const lastModified = normalizeLastModified(item.updatedAt ?? item.updated ?? item.lastModified ?? item.modified ?? fallbackLastModified);
  const revision = item.sequence ?? item.revision ?? item.version;
  const explicitSequence = Number(revision);
  const sequence = revision != null && Number.isInteger(explicitSequence) && explicitSequence >= 0 ? explicitSequence : sequenceFromLastModified(lastModified);
  return { status: normalizeEventStatus(sourceStatus || defaultStatus), sourceStatus, lastModified, sequence };
}

function withFingerprint(event) {
  return { ...event, revisionFingerprint: createEventFingerprint(event) };
}
...
function normalizeTv(result) {
  return result.episodes.map(episode => {
    ...
    return withFingerprint({
      ...
      sourceId: String(result.show.id), ...revisionFields(episode), metadata: { episodeId: String(id), season: episode.season, number: episode.number, network: episode.network }
    });
  }).filter(Boolean);
}
...
export function normalizeSportsStatus(value) { return normalizeEventStatus(value); }
...
function normalizeSports(result, requestedTeamId) {
  return result.events.map(event => {
    ...
    return withFingerprint({
      ...
      ...revisionFields(event),
      metadata: { eventId: String(id), league: event.league, homeTeam: event.homeTeam, awayTeam: event.awayTeam, broadcast: event.tvStation || null }
    });
  }).filter(Boolean);
}
...
function normalizeMovies(result) {
  return result.movies.map(movie => {
    ...
    return withFingerprint({
      uid: `makeics-movie-${stablePart(id)}@makeics`,
      ...
      source: result.query || 'All movies', sourceId: String(id), ...revisionFields(movie, 'CONFIRMED', result.generatedAt), metadata: { movieId: movie.id || null, genres: movie.genres || [] }
    });
  }).filter(Boolean);
}
...
export { createEventFingerprint, escapeIcsText } from './utils/ics.js';
...
export function toIcs(result, { now = new Date() } = {}) {
  ...
  for (const event of result.events) {
    ...
    lines.push(`SEQUENCE:${event.sequence ?? 0}`, `STATUS:${normalizeEventStatus(event.status)}`);
    if (event.sourceStatus) lines.push(`X-MAKEICS-STATUS:${escapeIcsText(event.sourceStatus.toUpperCase())}`);
    if (event.allDay) lines.push(`DTSTART;VALUE=DATE:${formatDate(event.start)}`, `DTEND;VALUE=DATE:${formatDate(event.end)}`);
    else lines.push(`DTSTART:${formatIcsUtc(event.start)}`, `DTEND:${formatIcsUtc(event.end)}`);
